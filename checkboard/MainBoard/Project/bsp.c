...
void Guide_EXTI_Control(u8 guide,u8 status);
 ...
 * @retvalue   :None  
******************************************************************************/
void TIM4_Configure(void)                                //TIME interupt          560us
{
	TIM_TimeBaseInitTypeDef TIM_TimeBaseStructure;
	NVIC_InitTypeDef NVIC_InitStructure;
...
/*****************************************************************************
 * @name       :void Guide_EXTI_Control(u8 guide,u8 status)
 * @date       :2020-09-19 
 * @author     :zhengbaiqiu 
 * @function   :
 * @parameters :0:左  1：右   2：前   3：上
 * @retvalue   :None  
******************************************************************************/
void Guide_EXTI_Control(u8 guide,u8 status)
{
...
//	NVIC_InitTypeDef   NVIC_InitStructure;
	...
	GPIO_InitStructure.GPIO_Pin = GPIO_Pin_2;//输入   
 	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_IPU;
 	GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;
 	GPIO_Init(GPIOD, &GPIO_InitStructure);
	...
	switch(guide)
	{
		...
		case 3:
			if(status == 1)
			{
				/* Connect EXTI11 Line to PD.2 pin */
...
// 	GPIO_InitStructure.GPIO_Pin = GPIO_Pin_13;//输入   
//  	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_IPU;
//  	GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;
//  	GPIO_Init(GPIOC, &GPIO_InitStructure);
	...
	EXTI_Init(&EXTI_InitStructure);
// 	
// 	/* Connect EXTI11 Line to PA.13 pin */
...
// 	EXTI_Init(&EXTI_InitStructure);
	...
	/* Connect EXTI12 Line to PD.2 pin */
	GPIO_EXTILineConfig(GPIO_PortSourceGPIOD, GPIO_PinSource2);
...
// 	mMaininf.mSystem.mSystem10usTick = 0;
	...
	mMaininf.mSystem.mSystem200usTick = 0;
	mMaininf.mSystem.mAvoidBumpStartTime = 0;
...
// 	mMaininf.mUart5.mReceiveFlag = 0;      //    没有接收导数据
	...
	mMaininf.mWork.mWorkAvoidBumpDat = 0;
	mMaininf.mWork.mWorkLeftAvoidBumpDatFar = 0;
	mMaininf.mWork.mWorkRightAvoidBumpDatFar = 0;
	mMaininf.mWork.mWorkAmongAvoidBumpDatFar = 0;
	
	mMaininf.mWork.mWorkLeftAlongSideDatFar = 0;
	mMaininf.mWork.mWorkLeftAlongSideDatNear = 0;
	mMaininf.mWork.mWorkRightAlongSideDatFar = 0;
	mMaininf.mWork.mWorkRightAlongSideDatNear = 0;
}
